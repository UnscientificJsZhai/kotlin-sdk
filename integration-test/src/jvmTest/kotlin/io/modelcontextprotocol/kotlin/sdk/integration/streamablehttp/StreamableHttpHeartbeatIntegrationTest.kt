...
private const val KTOR_HEARTBEAT_TYPE = "io.ktor.server.sse.Heartbeat"
...
class StreamableHttpHeartbeatIntegrationTest : AbstractStreamableHttpIntegrationTest() {
    ...
    @Test
    fun `legacy MCP over SSE APIs do not expose heartbeat configuration`() {
        val legacyMcpMethods = Class
            .forName("io.modelcontextprotocol.kotlin.sdk.server.KtorServerKt")
            .declaredMethods
            .filter { method -> method.name == "mcp" && Modifier.isPublic(method.modifiers) }

        legacyMcpMethods.size shouldBe 3
        legacyMcpMethods.any { method ->
            method.genericParameterTypes.any { type -> KTOR_HEARTBEAT_TYPE in type.typeName }
        } shouldBe false
    }
